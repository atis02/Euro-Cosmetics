...
export const DeliveryTime: FC<DeliveryCitiesProps> = ({
  selectedPaymentType,
  setSelectedPaymentType,
  errorFields,
}) => {
  const url = "/ordertimes/fetch/client";
  ...
  return (
    <Stack maxWidth={{ lg: "40vw", xs: "100%" }}>
      <Stack direction="row" flexWrap="wrap" gap={2}>
        {!isLoading &&
          !error &&
          data?.orderTimes?.length &&
          data.orderTimes?.map((type: DeliveryTypesI) => (
            <Button
              key={type.id}
              variant={
